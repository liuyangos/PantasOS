...
RUN --mount=type=cache,dst=/var/cache \
    ...
    if grep -q "kinoite" <<< "${BASE_IMAGE_NAME}"; then \
        ...
        dnf5 -y remove \
            ...
            plasma-discover \
            konsole && \
        sed -i '/<entry name="launchers" type="StringList">/,/<\/entry>/ s/<default>[^<]*<\/default>/<default>preferred:\/\/filemanager<\/default>/' /usr/share/plasma/plasmoids/org.kde.plasma.taskmanager/contents/config/main.xml && \
